...
# ---- OPTIMIZE EXPONENT ----
pythag_error <- function(exponent, data) {
  stats <- data %>%
    group_by(home_team) %>%
    summarise(home_scored = sum(home_runs, na.rm = TRUE),
              home_allowed = sum(away_runs, na.rm = TRUE),
              home_wins = sum(home_runs > away_runs, na.rm = TRUE),
              home_games = n()) %>%
    left_join(data %>%
                group_by(away_team) %>%
                summarise(away_scored = sum(away_runs, na.rm = TRUE),
                          away_allowed = sum(home_runs, na.rm = TRUE),
                          away_wins = sum(away_runs > home_runs, na.rm = TRUE),
                          away_games = n()),
    ...
    mutate(team = home_team,
           ...
           actual_win_pct = (home_wins + away_wins) / total_games,
           predicted_win_pct = total_scored^exponent / (total_scored^exponent + total_allowed^exponent))
  
  mean((stats$actual_win_pct - stats$predicted_win_pct)^2, na.rm = TRUE)
}

opt_result <- optimize(pythag_error, interval = c(0.5, 5), data = past_games)
optimal_exponent <- opt_result$minimum
cat("Optimal exponent: ", round(optimal_exponent, 4), "\n")

# Apply optimized exponent to Pythagorean win percentage
team_stats <- team_stats %>%
  mutate(pythag_win_pct = runs_scored^optimal_exponent / 
           (runs_scored^optimal_exponent + runs_allowed^optimal_exponent))
# ---- END OPTIMIZE EXPONENT ----


# Compute league average runs
...
team_stats <- team_stats %>%
  mutate(OS = runs_scored / games_played / league_avg_runs, 
         DS = runs_allowed / games_played / league_avg_runs)
